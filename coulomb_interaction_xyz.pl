...
open(OUT,">coulomb_interaction_xyz.out") or die ":( coulomb_interaction_xyz.out";
...
print "\n";
$file1_c=which_file("File name of the G09-log with charges for this fragment: ");
print OUT "Fragment-1 charge:  $file1_c\n";
print "\n";
$file2=which_file("Fragment-2 XYZ input file name: ");
print OUT "Fragment-2 geom:    $file2\n";
print "\n";
$file2_c=which_file("File name of the G09-log with charges for this fragment: ");
print OUT "Fragment-2 charge:  $file2_c\n";
print "\n";

# Charge type
print " Charge type (Mulliken - 0, NBO - 1, ChelpG - 2; default: 1): ";
$_=<STDIN>;
chomp;$_ =~ s/^\s*//;$_ =~ s/\s*$//;
$type=$_;
if ($type eq ""){
  ...
  print OUT "Using Mulliken\n";
}elsif($type == 1){
  print OUT "Using NBO...\n";
}elsif($type == 2){
  die "\n\n ChelpG still not implemented.\n\n";
}else{
  die "\n\n type=$type: Charge type unknown.\n\n";
}
print "\n";

# Electric constant
print " Electric constant (au, default: vacuum = 1.0): ";
$_=<STDIN>;
chomp;$_ =~ s/^\s*//;$_ =~ s/\s*$//;
$eps=$_;
if ($eps eq ""){
...
print OUT "Electric constant (au): $eps\n\n";
print "\n";
...
# Read charge fragment-1
open(INP,$file1_c) or die ":( $file1_c";
while(<INP>){
  if ($type == 0){
    if (/Mulliken atomic charges:/){
       ...
       for ($i = 0; $i <= $nat1-1; $i++){
         ...
         (@g)=split(/\s+/,$_); 
         $q1[$i]=$g[2];
         printf OUT "Fragment-1 charge %3d: %8.4f\n",$i,$q1[$i]; 
       }
       last;
    }
  }
...
open(INP,$file2_c) or die ":( $file2_c";
while(<INP>){
  if ($type == 0){
    if (/Mulliken atomic charges:/){
       $_=<INP>;
       for ($i = 0; $i <= $nat2-1; $i++){
         ...
         (@g)=split(/\s+/,$_);
         $q2[$i]=$g[2];
         printf OUT "Fragment-2 charge %3d: %8.4f\n",$i,$q2[$i];
       }
       last;
    }
  }
...
for ($i = 0; $i <= $nat1-1; $i++){
  for ($j = 0; $j <= $nat2-1; $j++){
     $dist_A = sqrt(($x1[$i]-$x2[$j])**2+($y1[$i]-$y2[$j])**2+($z1[$i]-$z2[$j])**2);
     $dist = $dist_A*$ang2bohr;
     $vch = $eps*$q1[$i]*$q2[$j]/$dist;
     $VC = $VC + $vch;
     printf OUT "Pair: %3d %3d    dist \(A\): %8.4f   Vc \(hartree\): %12.6f\n",$i,$j,$dist_A,$vch;
  }
}
$VCe=$VC*$au2ev;
...
$MCG=$eps*$tq1*$tq2/($distg*$ang2bohr);
$MCGe=$MCG*$au2ev;

# Monopole interaction CM
$distcm=sqrt(($x1cm-$x2cm)**2+($y1cm-$y2cm)**2+($z1cm-$z2cm)**2);
$MC=$eps*$tq1*$tq2/($distcm*$ang2bohr);
$MCe=$MC*$au2ev;

# Monopole interaction CC
$distq=sqrt(($x1q-$x2q)**2+($y1q-$y2q)**2+($z1q-$z2q)**2);
$MCC=$eps*$tq1*$tq2/($distq*$ang2bohr);
$MCCe=$MCC*$au2ev;

printf OUT "\nFragment 1 - Number of atoms: %4d     Total Mass (amu): %12.3f   Total charge (amu): %5.2f\n",$nat1,$mass1,$tq1;
printf OUT "             Center of mass (A):   %10.5f  %10.5f  %10.5f \n",$x1cm,$y1cm,$z1cm;
printf OUT "             Center of charge (A): %10.5f  %10.5f  %10.5f \n",$x1q,$y1q,$z1q;
printf OUT "             Geometric center (A): %10.5f  %10.5f  %10.5f \n",$x1g,$y1g,$z1g;
printf OUT   "Fragment 2 - Number of atoms: %4d     Total Mass (amu): %12.3f   Total charge (amu): %5.2f\n",$nat2,$mass2,$tq2;
printf OUT "             Center of mass (A):   %10.5f  %10.5f  %10.5f \n",$x2cm,$y2cm,$z2cm;
printf OUT "             Center of charge (A): %10.5f  %10.5f  %10.5f \n",$x2q,$y2q,$z2q;
printf OUT "             Geometric center (A): %10.5f  %10.5f  %10.5f \n",$x2g,$y2g,$z2g;
printf OUT "Distance between center of masses (A):  %10.5f\n",$distcm;
printf OUT "Distance between center of charges (A): %10.5f\n",$distq;
printf OUT "Distance between geometric centers (A): %10.5f\n",$distg;
printf OUT "Total Coulomb interaction:                %12.6f hartree    %8.3f eV\n",$VC,$VCe;
printf OUT "Monopole interaction (center of mass):    %12.6f hartree    %8.3f eV\n",$MC,$MCe;
printf OUT "Monopole interaction (center of charges): %12.6f hartree    %8.3f eV\n",$MCC,$MCCe;
printf OUT "Monopole interaction (geometric center):  %12.6f hartree    %8.3f eV\n\n",$MCG,$MCGe;
