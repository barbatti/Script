...
use POSIX;
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# This program computes TD-Baeck-An couplings for an ensemble of trajectories results.
# The input parameters should be changed in the next block.
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
$smooth = 0.4;  # 0  - direct numerical derivative
                # >0 - analytical derivative of quadratic regression with smooth/dt points
		# smooth should be given in fs.  

$clean  = 0.1;  # If |vdoh(t)| - |vdoth(t-Dt)| > clean, vdoh(t) = vdoh(t-Dt)
                # 0.1 au is a reasonable value

$deps   = 12;   # Coupling is zero if |DE| > deps (eV)
$deh    = 1;    # Coupling is zero if |DE*DE''| > deh (au)

$traji  = 14;    # Initial trajectory
$trajf  = 14;  # Final trajectory
...
sub baeck_an{
     ...
     for ($k=2 ; $k<=$nstat; $k++){
	  ...
	  if ($absv == 0){
	    $nac = $nac." ".sprintf("%9.6f",$sgm[$ns][$j][$k])." ".sprintf("%9.6f",$vdh[$ns][$j][$k]);
          }elsif($absv == 1){
...
sub second_derivative{
  ...
  if ($ns-$nreset == 2){
    ...
    if ($smooth == 0){
      ...
      if ($npoints > $ns-$nreset){
	$np = $ns-$nreset;
      }else{
	$np = $npoints;
      } 
      for ($ip=0;$ip<=$np-1;$ip++){
	$y[$ip] = $de[$ns-$np+1+$ip][$j][$k];
      }
      #print LOG "y = @y\n";
...
sub sigma{
  ...
  if (($q <= 0) or (abs($de[$ns][$j][$k]) > $deps) or (abs($de[$ns][$j][$k]*$d2E) > $deh)){
    $sgm[$ns][$j][$k] = 0.0;
  }elsif(($q > 0) and (abs($de[$ns][$j][$k]) <= $deps) and (abs($de[$ns][$j][$k]*$d2E) <= $deh)){
