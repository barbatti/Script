...
#
# If dyn.xyz is not present, it uses $NX to get it.
#
#
$nx = $ENV{"NX"}; 
...
sub read_parameters{
  ...
  # xyz files
  for ($it=$itraj;$it<=$ftraj;$it++){
    if (!-s "TRAJ$it/RESULTS/dyn.xyz"){
      chdir("TRAJ$it/RESULTS");
      system("$nx/dynout2xyz.pl");
      chdir("../../");
    }
  }
  # Number of atoms
  $nat=0;
  $dmld="TRAJ$itraj/RESULTS/dyn.xyz"; 
  if (-s $dmld){
...
sub criteria_2{
  ...
  #     $criterium = "Y";  print STDOUT "XXXX backhop\n";
     }
   }else
   {
# print STDOUT "entering crit 2 $it iteration, time  $time[0] surfaces $surf[0] and $surf[1] \n ";
   if (($surf[0] == $is) and ($surf[1] == $fs)){
       $criterium = "Y";
 #      print STDOUT "crit 2 was met, line  ";
     }
   }
   if ($criterium eq "Y"){
