#!/usr/bin/perl -w

# Distance between two geometries d = |Ri-R0|
# were Ri are the cartesian coordinates (xyz format) of ith
# geometry in dyn.mld. R0 is the reference geometry written 
# to geom.molden (also xyz format).
#
# Mario Barbatti, Sept. 2007

open(LOG,">distance.log") or die ":( distance.log!";
open(DAT,">distance.dat") or die ":( distance.dat!";
open(DMW,">distance-mw.dat") or die ":( distance-mw.dat!";

print LOG "    :: DISTANCE PROGRAM ::\n\n";

$file_geom="dyn.mld";
$file_ref = "geom.molden";

reference();
geometry();

close(LOG);

#end main program

sub distance{
  $sum=0.0;
  $sum_mw=0.0;
  for ($i=1;$i<=$nat;$i++){
    $dr=($x[$i]-$xr[$i])**2+($y[$i]-$yr[$i])**2+($z[$i]-$zr[$i])**2;
    $drmw=$mass[$i]*(($x[$i]-$xr[$i])**2+($y[$i]-$yr[$i])**2+($z[$i]-$zr[$i])**2);
    $sum=$sum+$dr;
    $sum_mw=$sum_mw+$drmw;
  }
  $dist=sqrt($sum);
  $dist_mw=sqrt($sum_mw);
  print LOG " Distance GEOM_$n - REF = $dist Angstrom ($dist_mw amu^1/2*Angstrom) \n\n";
  printf DAT " %4d    %7.3f   %7.3f\n",$n,$dist,$dist_mw;
  printf DMW "%6.2f\n",$dist_mw;
}

sub geometry{
 open(FG,$file_geom) or die ":( $file_geom";
 $n = 0;
 while(<FG>){
   $n++;
   print LOG " Geometry $n \n";
   $_=<FG>;
   for ($i=1;$i<=$nat;$i++){
     $_=<FG>;
     chomp;
     $_ =~ s/^\s*//;         # remove leading blanks
     $_ =~ s/\s*$//;         # remove trailing blanks
     ($s[$i],$x[$i],$y[$i],$z[$i])=split(/\s+/,$_);
     print LOG " $s[$i]  $x[$i]  $y[$i]  $z[$i] \n";
   }
   distance();
 }
}

sub reference{
 # number of atoms
 open(FR,$file_ref) or die ":( $file_ref";
 $_=<FR>;
 chomp;
 $_ =~ s/^\s*//;         # remove leading blanks
 $_ =~ s/\s*$//;         # remove trailing blanks
 $nat=$_;
 print LOG " Number of atoms is: $nat \n";
 # Reference geometry
 print LOG " Reference geometry: \n";
 $_=<FR>;
 for ($i=1;$i<=$nat;$i++){
   $_=<FR>;
   chomp;
   $_ =~ s/^\s*//;         # remove leading blanks
   $_ =~ s/\s*$//;         # remove trailing blanks
  ($sr[$i],$xr[$i],$yr[$i],$zr[$i])=split(/\s+/,$_);
  print LOG " $sr[$i]  $xr[$i]  $yr[$i]  $zr[$i] \n";
  which_mass();
  $mass[$i]=$atomic_mass;
 }
  print LOG "\n";
 close(FR);
}

sub which_mass{
        if ($sr[$i]=~m/x/i){
          $numz  = 0;
          $atomic_mass = 0.0;
        }
        if ($sr[$i]=~m/h/i){
          $numz  = 1;
          $atomic_mass = 1.007825037;
        }
        if ($sr[$i]=~m/he/i){
          $numz  = 2;
          $atomic_mass = 4.00260325;
        }
        if ($sr[$i]=~m/li/i){
          $numz  = 3;
          $atomic_mass = 7.0160045;
        }
        if ($sr[$i]=~m/be/i){
          $numz  = 4;
          $atomic_mass = 9.0121825;
        }
        if ($sr[$i]=~m/b/i){
          $numz  = 5;
          $atomic_mass = 11.0093053;
        }
        if ($sr[$i]=~m/c/i){
          $numz  = 6;
          $atomic_mass = 12.0;
        }
        if ($sr[$i]=~m/n/i){
          $numz  = 7;
          $atomic_mass = 14.003074008;
        }
        if ($sr[$i]=~m/o/i){
          $numz  = 8;
          $atomic_mass = 15.99491464;
        }
        if ($sr[$i]=~m/f/i){
          $numz  = 9;
          $atomic_mass = 18.99840325;
        }
        if ($sr[$i]=~m/ne/i){
          $numz  = 10;
          $atomic_mass = 19.9924391;
        }
        if ($sr[$i]=~m/na/i){
          $numz  = 11;
          $atomic_mass = 22.9897697;
        }
        if ($sr[$i]=~m/mg/i){
          $numz  = 12;
          $atomic_mass = 23.9850450;
        }
        if ($sr[$i]=~m/al/i){
          $numz  = 13;
          $atomic_mass = 26.9815413;
        }
        if ($sr[$i]=~m/si/i){
          $numz  = 14;
          $atomic_mass = 27.9769284;
        }
        if ($sr[$i]=~m/p/i){
          $numz  = 15;
          $atomic_mass = 30.9737634;
        }
        if ($sr[$i]=~m/s/i){
          $numz  = 16;
          $atomic_mass = 31.9720718;
        }
        if ($sr[$i]=~m/cl/i){
          $numz  = 17;
          $atomic_mass = 34.968852729;
        }
        if ($sr[$i]=~m/ar/i){
          $numz  = 18;
          $atomic_mass = 39.9623831;
        }
        if ($sr[$i]=~m/k/i){
          $numz  = 19;
          $atomic_mass = 38.9637079;
        }
        if ($sr[$i]=~m/ca/i){
          $numz  = 20;
          $atomic_mass = 39.9625907;
        }
        if ($sr[$i]=~m/sc/i){
          $numz  = 21;
          $atomic_mass = 44.9559136;
        }
        if ($sr[$i]=~m/ti/i){
          $numz  = 22;
          $atomic_mass = 47.9479467;
        }
        if ($sr[$i]=~m/v/i){
          $numz  = 23;
          $atomic_mass = 50.9439625;
        }
        if ($sr[$i]=~m/cr/i){
          $numz  = 24;
          $atomic_mass = 51.9405097;
        }
        if ($sr[$i]=~m/mn/i){
          $numz  = 25;
          $atomic_mass = 54.9380463;
        }
        if ($sr[$i]=~m/fe/i){
          $numz  = 26;
          $atomic_mass = 55.9349393;
        }
        if ($sr[$i]=~m/co/i){
          $numz  = 27;
          $atomic_mass = 58.9331978;
        }
        if ($sr[$i]=~m/ni/i){
          $numz  = 28;
          $atomic_mass = 57.9353471;
        }
        if ($sr[$i]=~m/cu/i){
          $numz  = 29;
          $atomic_mass = 62.9295992;
        }
        if ($sr[$i]=~m/zn/i){
          $numz  = 30;
          $atomic_mass = 63.9291454;
        }
        if ($sr[$i]=~m/ga/i){
          $numz  = 31;
          $atomic_mass = 68.9255809;
        }
        if ($sr[$i]=~m/ge/i){
          $numz  = 32;
          $atomic_mass = 73.9211788;
        }
        if ($sr[$i]=~m/as/i){
          $numz  = 33;
          $atomic_mass = 74.9215955;
        }
        if ($sr[$i]=~m/se/i){
          $numz  = 34;
          $atomic_mass = 79.9165205;
        }
        if ($sr[$i]=~m/br/i){
          $numz  = 35;
          $atomic_mass = 78.9183361;
        }
        if ($sr[$i]=~m/kr/i){
          $numz  = 36;
          $atomic_mass = 83.80;
        }
        if ($sr[$i]=~m/rb/i){
          $numz  = 37;
          $atomic_mass = 85.4678;
        }
        if ($sr[$i]=~m/sr/i){
          $numz  = 38;
          $atomic_mass = 87.62;
        }
        if ($sr[$i]=~m/y/i){
          $numz  = 39;
          $atomic_mass = 88.9059;
        }
        if ($sr[$i]=~m/zr/i){
          $numz  = 40;
          $atomic_mass = 91.22;
        }
        if ($sr[$i]=~m/nb/i){
          $numz  = 41;
          $atomic_mass = 92.9064;
        }
        if ($sr[$i]=~m/mo/i){
          $numz  = 42;
          $atomic_mass = 95.94;
        }
        if ($sr[$i]=~m/tc/i){
          $numz  = 43;
          $atomic_mass = 98;
        }
        if ($sr[$i]=~m/ru/i){
          $numz  = 44;
          $atomic_mass = 101.07;
        }
        if ($sr[$i]=~m/rh/i){
          $numz  = 45;
          $atomic_mass = 102.9055;
        }
        if ($sr[$i]=~m/pd/i){
          $numz  = 46;
          $atomic_mass = 106.4;
        }
        if ($sr[$i]=~m/ag/i){
          $numz  = 47;
          $atomic_mass = 107.868;
        }
        if ($sr[$i]=~m/cd/i){
          $numz  = 48;
          $atomic_mass = 112.41;
        }
        if ($sr[$i]=~m/in/i){
          $numz  = 49;
          $atomic_mass = 114.82;
        }
        if ($sr[$i]=~m/sn/i){
          $numz  = 50;
          $atomic_mass = 118.69;
        }
        if ($sr[$i]=~m/sb/i){
          $numz  = 51;
          $atomic_mass = 121.75;
        }
        if ($sr[$i]=~m/te/i){
          $numz  = 52;
          $atomic_mass = 127.60;
        }
        if ($sr[$i]=~m/i/i){
          $numz  = 53;
          $atomic_mass = 126.9045;
        }
        if ($sr[$i]=~m/xe/i){
          $numz  = 54;
          $atomic_mass = 131.30;
        }
        if ($sr[$i]=~m/cs/i){
          $numz  = 55;
          $atomic_mass = 132.9054;
        }
        if ($sr[$i]=~m/ba/i){
          $numz  = 56;
          $atomic_mass = 137.33;
        }
        if ($sr[$i]=~m/la/i){
          $numz  = 57;
          $atomic_mass = 138.9055;
        }
        if ($sr[$i]=~m/ce/i){
          $numz  = 58;
          $atomic_mass = 140.12;
        }
        if ($sr[$i]=~m/pr/i){
          $numz  = 59;
          $atomic_mass = 140.9077;
        }
        if ($sr[$i]=~m/nd/i){
          $numz  = 60;
          $atomic_mass = 144.24;
        }
        if ($sr[$i]=~m/pm/i){
          $numz  = 61;
          $atomic_mass = 145;
        }
        if ($sr[$i]=~m/sm/i){
          $numz  = 62;
          $atomic_mass = 150.4;
        }
        if ($sr[$i]=~m/eu/i){
          $numz  = 63;
          $atomic_mass = 151.96;
        }
        if ($sr[$i]=~m/gd/i){
          $numz  = 64;
          $atomic_mass = 157.25;
        }
        if ($sr[$i]=~m/tb/i){
          $numz  = 65;
          $atomic_mass = 158.9254;
        }
        if ($sr[$i]=~m/dy/i){
          $numz  = 66;
          $atomic_mass = 162.50;
        }
        if ($sr[$i]=~m/ho/i){
          $numz  = 67;
          $atomic_mass = 164.9304;
        }
        if ($sr[$i]=~m/er/i){
          $numz  = 68;
          $atomic_mass = 167.26;
        }
        if ($sr[$i]=~m/tm/i){
          $numz  = 69;
          $atomic_mass = 168.9342;
        }
        if ($sr[$i]=~m/yb/i){
          $numz  = 70;
          $atomic_mass = 173.04;
        }
        if ($sr[$i]=~m/lu/i){
          $numz  = 71;
          $atomic_mass = 174.967;
        }
        if ($sr[$i]=~m/hf/i){
          $numz  = 72;
          $atomic_mass = 178.49;
        }
        if ($sr[$i]=~m/ta/i){
          $numz  = 73;
          $atomic_mass = 180.9479;
        }
        if ($sr[$i]=~m/w/i){
          $numz  = 74;
          $atomic_mass = 183.85;
        }
        if ($sr[$i]=~m/re/i){
          $numz  = 75;
          $atomic_mass = 186.207;
        }
        if ($sr[$i]=~m/os/i){
          $numz  = 76;
          $atomic_mass = 190.2;
        }
        if ($sr[$i]=~m/ir/i){
          $numz  = 77;
          $atomic_mass = 192.22;
        }
        if ($sr[$i]=~m/pt/i){
          $numz  = 78;
          $atomic_mass = 195.09;
        }
        if ($sr[$i]=~m/au/i){
          $numz  = 79;
          $atomic_mass = 196.9665;
        }
        if ($sr[$i]=~m/hg/i){
          $numz  = 80;
          $atomic_mass = 200.59;
        }
        if ($sr[$i]=~m/tl/i){
          $numz  = 81;
          $atomic_mass = 204.37;
        }
        if ($sr[$i]=~m/pb/i){
          $numz  = 82;
          $atomic_mass = 207.2;
        }
        if ($sr[$i]=~m/bi/i){
          $numz  = 83;
          $atomic_mass = 208.9804;
        }
        if ($sr[$i]=~m/po/i){
          $numz  = 84;
          $atomic_mass = 209;
        }
        if ($sr[$i]=~m/at/i){
          $numz  = 85;
          $atomic_mass = 210;
        }
        if ($sr[$i]=~m/rn/i){
          $numz  = 86;
          $atomic_mass = 222;
        }
        if ($sr[$i]=~m/fr/i){
          $numz  = 87;
          $atomic_mass = 223;
        }
        if ($sr[$i]=~m/ra/i){
          $numz  = 88;
          $atomic_mass = 226.0254;
        }
        if ($sr[$i]=~m/ac/i){
          $numz  = 89;
          $atomic_mass = 227.0278;
        }
        if ($sr[$i]=~m/th/i){
          $numz  = 90;
          $atomic_mass = 232.0381;
        }
        if ($sr[$i]=~m/pa/i){
          $numz  = 91;
          $atomic_mass = 231.0359;
        }
        if ($sr[$i]=~m/u/i){
          $numz  = 92;
          $atomic_mass = 238.029;
        }
        if ($sr[$i]=~m/np/i){
          $numz  = 93;
          $atomic_mass = 237.0482;
        }
        if ($sr[$i]=~m/pu/i){
          $numz  = 94;
          $atomic_mass = 244;
        }
        if ($sr[$i]=~m/am/i){
          $numz  = 95;
          $atomic_mass = 243;
        }
        if ($sr[$i]=~m/cm/i){
          $numz  = 96;
          $atomic_mass = 247;
        }
        if ($sr[$i]=~m/bk/i){
          $numz  = 97;
          $atomic_mass = 247;
        }
        if ($sr[$i]=~m/cf/i){
          $numz  = 98;
          $atomic_mass = 251;
        }
        if ($sr[$i]=~m/es/i){
          $numz  = 99;
          $atomic_mass = 254;
        }
        if ($sr[$i]=~m/fm/i){
          $numz  = 100;
          $atomic_mass = 257;
        }
        if ($sr[$i]=~m/md/i){
          $numz  = 101;
          $atomic_mass = 258;
        }
        if ($sr[$i]=~m/no/i){
          $numz  = 102;
          $atomic_mass = 259;
        }
        if ($sr[$i]=~m/lr/i){
          $numz  = 103;
          $atomic_mass = 260;
        }
}









