...
print STDOUT "5 - (phy>180)->(phy<180) and theta -> 180-theta \n";
...
print OUT "    n time    Q      phi     theta conf phi_p  theta_p conf_p\n";
while(<IN>){
  chomp;$_ =~ s/^\s*//;$_ =~ s/\s*$//;
  ($n,$time,$q,$theta,$phy,$conf)=split(/\s+/,$_);
  if ($n=~/^-?\d+$/){
  if ($option == 1){
    option1();
  }elsif($option==2){
  ...
  printf OUT "%5d %6.2f %6.3f %6.1f %6.1f %4s %6.1f %6.1f  %4s \n",$n,$time,$q,$phy,$theta,$conf,$phy_p,$theta_p,$class;
  }
}
#
sub option1{
