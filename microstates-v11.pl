...
sub read_input{
  ...
  if ($de == 0){
    print LOG "\nde = $de: de is assumed to be f[0]=$f[0] au.\n";
    $de = $f[0];
  }

  if ($nmax == 0){
    print LOG "For nmax = 0, the maximum number of quanta in each mode is:\n"; 
    for ($i=0; $i<=$imax-1; $i++){
      $nqmax[$i] = ceil(($emax-$zpe)/$f[$i]);
      print LOG "nqmax[$i] = $nqmax[$i]\n"; 
    }
  }else{
    for ($i=0; $i<=$imax-1; $i++){
      $nqmax[$i] = $nmax;
...
sub process_mst{
  ...
  if ($e <= $emax){
    if ($list_om =~ /y/i){	  
      if ($imax == 3){	    
        $result1 = $result1.sprintf("%3d %3d %3d %12.6f\n",$n[2],$n[1],$n[0],$e);
      }elsif($imax == 4){
        $result1 = $result1.sprintf("%3d %3d %3d %3d %12.6f\n",$n[3],$n[2],$n[1],$n[0],$e);
      }elsif($imax == 5){
        $result1 = $result1.sprintf("%3d %3d %3d %3d %3d %12.6f\n",$n[4],$n[3],$n[2],$n[1],$n[0],$e);
      }elsif($imax == 6){
        $result1 = $result1.sprintf("%3d %3d %3d %3d %3d %3d %12.6f\n",$n[5],$n[4],$n[3],$n[2],$n[1],$n[0],$e);
      }elsif($imax == 7){
        $result1 = $result1.sprintf("%3d %3d %3d %3d %3d %3d %3d %12.6f\n",$n[6],$n[5],$n[4],$n[3],$n[2],$n[1],$n[0],$e);
      }elsif($imax == 8){
        $result1 = $result1.sprintf("%3d %3d %3d %3d %3d %3d %3d %3d %12.6f\n",$n[7],$n[6],$n[5],$n[4],$n[3],$n[2],$n[1],$n[0],$e);
      }elsif($imax == 9){
        $result1 = $result1.sprintf("%3d %3d %3d %3d %3d %3d %3d %3d %3d %12.6f\n",$n[8],$n[7],$n[6],$n[5],$n[4],$n[3],$n[2],$n[1],$n[0],$e);
      }
    }
    $energy[$k] = $e;
...
sub entropy{
  ...
  for ($i = 0; $i <= $n_bins; $i++){
    ...
    if ($i == 0){
      ...
      $invT = ($S[$i+1]+$S[$i]-2.0*$S[$i-1])/$de;                        # assymetric 2nd-order
    }elsif(($i >= 2) and ($i < $n_bins-1)){
      $invT = (-$S[$i+2]+8.0*$S[$i+1]-8.0*$S[$i-1]+$S[$i-2])/(12.0*$de); # centered 4th-order
    }elsif($i == $n_bins-1){
      $invT = ($S[$i+1]-$S[$i-1])/(2.0*$de);                             # centered 2nd-order
    }elsif($i == $n_bins){
      $invT = (3.0*$S[$i]-4.0*$S[$i-1]+$S[$i-2])/(2.0*$de);              # backward 2nd-order
    }
    ...
    if ($i == 0){
      ...
      printf OUT2 "%10.6f  %6d  %10.4e  %10.4e  %9.1f\n",$e_bin,$Omega[$i],$S[$i],$invT,$T;
      printf OUT4 "%10.6f  %10.6f  %10.6f  %10.6f  %6d  %9.1f\n",$e_bin,$zpe,$f[0],$f[$imax-1],$imax,$T;
    }
    printf OUT3 "%10.6f  %10.6f  %10.6f  %10.6f  %6d  %8d\n",$e_bin,$zpe,$f[0],$f[$imax-1],$imax,$Omega[$i];
