...
for ($k=0; $k<=$Np; $k++){
  ...
  getint();
  system("cp -rf cart2intls int2cartls-$k");
  ...
  system("cd $dispdir/$coldir/ ; $mld/nx2tm ; $mld/nx2xyz ; cd ../../");
  print DP "1   $k\n";
}

system("$AD/distance-curve.pl");
 system("rm -f intgeom* int2cart* geom geom.new cart2int* liic.inp bummer bmatrix cartgrd");
 system("rm -f int2cart* geom geom.new cart2int* liic.inp bummer bmatrix cartgrd");
