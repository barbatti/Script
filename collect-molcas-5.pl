...
while(<DP>){ 
  ...
  if (-s $dmf){
    $dist[$i]=<DM>;
    chomp($dist[$i]);$dist[$i] =~ s/^\s*//;$dist[$i] =~ s/\s*$//;
  }else{
    $dist[$i]=$i;
  }
  find_energy();
